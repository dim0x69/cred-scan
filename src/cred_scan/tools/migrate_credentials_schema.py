...
before running it against an existing workspace directory. Every document and
...
def _validate_document(payload: dict[str, Any], path: Path) -> CredentialsDocument:
    # This historical utility writes schema 3 with scope_id. Build a current
    # in-memory validation view without making the old output runtime-readable.
    try:
...
def migrate_workspace(workspace_dir: Path, *, dry_run: bool = False) -> int:
    ...
    for path in sorted(workspace_dir.rglob("credentials.json")):
...
def main() -> int:
    ...
    count = migrate_workspace(args.workspace_dir, dry_run=args.dry_run)
